...
class TestUsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create test_user" do
    assert_difference('TestUser.count') do
      post test_users_url, params: { test_user: { desc: @test_user.desc, email: @test_user.email, guid: @test_user.guid, status: @test_user.status } }
    end
  ...
  test "should update test_user" do
    patch test_user_url(@test_user), params: { test_user: { desc: @test_user.desc, email: @test_user.email, guid: @test_user.guid, status: @test_user.status } }
